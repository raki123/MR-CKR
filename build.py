...
def change_and_basic_from_ontology(onto, exchangeable_concepts):
    ...
    concepts = [ concept for concept in concepts if any(name in concept for name in exchangeable_concepts) ]
    # add extra concepts
    original_concepts = []
    ...
    return (change_context, basic_context)


onto = Graph()
onto = onto.parse("with_scene.owl")
exchangeable_concepts = [ "Child", "GlidingOnWheels", "RollingContainer", "Sign", "Smoke", "StopLineMarking", "Human", "Car" ]
change_context, basic_context = change_and_basic_from_ontology(onto, exchangeable_concepts)
...
with open("./MR_CKR/extra.lp", 'a') as extra_constraints:
